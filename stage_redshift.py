...
class StageToRedshiftOperator(BaseOperator):
    ...
    """ Operator that loads data from S3 to Redshift
        ...
        :param aws_conn_id: Connection ID for aws
    ...
    def execute(self, context):
        ...
        self.log.info(f"Connection with Redshift successfully completed with aws credentials.")
        ...
        self.log.info(f"Using this S3 path: {s3_path}")

        self.log.info('Loading data from {} to Redshift table {}'.format(s3_path, self.table))

        self.log.info(f"Start copying data to staging table: {self.table}")
        ...
        self.log.info(f"Copying finished for table: {self.table}")
